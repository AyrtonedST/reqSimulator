    ...
    @if (!loading()) {
      <div class="app-container">
        <div class="card">
          ...
          <p class="subtitle">Selecciona un endpoint para interactuar con la API</p>
...
export class App {
  ...
  async handleClick(endpoint: string) {
    ...
    this.response.set('');
    this.error.set(null);
    const traceTag = (window as any).dtrum?.traceId;
    try {
      const headers = new Headers();
      headers.set('Content-Type', 'application/json');
      if (traceTag) {
        headers.set('x-dt-trace-tag', traceTag);
      }
      const res = await fetch(`${this.CLOUDRUN_API_URL}${endpoint}`, { headers });
      if (!res.ok) {
        throw new Error(`HTTP error! status: ${res.status}`);
      }
